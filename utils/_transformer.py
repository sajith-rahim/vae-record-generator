...
class Transformer:
    ...
    def __init__(self, n_modes=3, weightage=0.005):
    ...
    def transform_discrete(self, attr_transform_details, raw_column_data):
        ...
        return [ohe.transform(raw_column_data).toarray()]
    ...
    def inverse_transform_discrete(self, attr_transform_info, attr_data):
        ohe = attr_transform_info.transformer
        reversed_data = []
        all_categories = list(itertools.chain(*ohe.categories_))

        for _row_index, feature_index in zip(*attr_data.nonzero()):
