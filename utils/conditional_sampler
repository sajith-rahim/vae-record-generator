...
class ConditionalSampler:
    ...
    def __init__(self, data, output_info, log_frequency):
        ...
        # For example _rowid_by_cat_cols[a][b] is a list of all rows with the
        # a-th discrete column equal value b.
        self._rowid_by_cat_cols = []

        # Compute _rowid_by_cat_cols
        st = 0
        for column_info in output_info:
            if self.is_discrete_column(column_info):
                ...
                for j in range(span_info.dim):
                    rowid_by_cat.append(np.nonzero(data[:, st + j])[0])
                self._rowid_by_cat_cols.append(rowid_by_cat)
                st = ed
            else:
        ...
        max_category = max(
            [column_info[0].dim for column_info in output_info
             if self.is_discrete_column(column_info)], default=0)
    ...
    def sample_conditional_vec(self, batch):
        ...
        if self._n_discrete_columns == 0:
            return None
        # randomly select a discrete column with equal prob
        discrete_column_id = np.random.choice(
            np.arange(self._n_discrete_columns), batch)

        cond = np.zeros((batch, self._n_categories), dtype='float32')
        # zero-filled mask vector
        mask = np.zeros((batch, self._n_discrete_columns), dtype='float32')
        # kth comp of ith mask
    ...
    def sample_original_conditional_vector(self, batch):
        """
    ...
    def sample_data(self, n, col, opt):
        ...
        for c, o in zip(col, opt):
            idx.append(np.random.choice(self._rowid_by_cat_cols[c][o]))
    ...
    def generate_cond_from_condition_column_info(self, condition_info, batch):
        _vector = np.zeros((batch, self._n_categories), dtype='float32')
